...
public class ProductsDAO {
    ...
    public static int save(Products p) throws SQLException {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("insert into products(label,price,onStock)values(?,?,?)");

                ps.setString(1,p.getLabel());
                ps.setInt(2,p.getPrice());
                ps.setInt(3,p.getOnStock());
...
int status=0;
    ...
    public static int update(Products p) throws SQLException {
        int status= 0;
        try{Connection conn = ProductsDAO.getConnection();
            PreparedStatement ps = conn.prepareStatement("update products set label=?,price=?,onStock=? where product_id=?");
            ps.setString(1,p.getLabel());
            ps.setInt(2,p.getPrice());
            ps.setInt(3,p.getOnStock());
